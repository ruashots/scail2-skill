...
1. **FIRST-FRAME MATCH (the #1 rule).** Results are best when the reference person is **framed and posed like the subject in the driving clip's FIRST FRAME** — same scale (full-body↔full-body, headshot↔headshot), same rough position/pose. A big pose/scale gap at frame 1 = a soft, muddy result. The engine prints a warning on gross aspect mismatch, but it cannot fix framing — supply a matched reference. To make one: extract the clip's first frame (`ffmpeg -i CLIP.mp4 -frames:v 1 first.png`) and pick/generate a reference matched to it (the `ideogram4` skill can generate a pose/framing-matched reference).
